...
def get_argparser():
    ...
    #Experiment Note
    parser.add_argument('--note',  type=str, default='all_rows, left_main, right_main',
...
def main():
    ...
    for cur_epoch in range(opts.epoch_num):
        ...
        for (images, labels) in train_loader:
            ...
            lbls = torch.concat([lbl3, lbl1, lbl2], 1) #[2 2 256 256]
            loss = criterion(outputs[:,:3], lbls)
            ...
            # loss = 1*loss1 + 1*loss2 + 1*loss3 + 0*loss4
        ...
        if (cur_epoch) % opts.val_interval == 0:
            ...
            if val_score[1]['Mean IoU'] > best_score:  # when main_row best
