...
def get_argparser():
    ...
    parser.add_argument('--demo_img',  type=str, default='/home/nvidia/projext/ceshi.jpg',
                        help='the path of the demo image')
    parser.add_argument('--ckpt_path',  type=str, default='/home/nvidia/projext/best_AttU_Net.pth',
...
def watch_thin(image, mask2, mask3, mask4):
    table = table_cmp()
    mask_2 = table[mask2*2].astype(np.uint8)
    mask_3 =table[mask3*3].astype(np.uint8)
    mask_4 = table[mask4*4].astype(np.uint8)
    ...
    ax.xaxis.set_major_locator(matplotlib.ticker.NullLocator())
    ax.yaxis.set_major_locator(matplotlib.ticker.NullLocator())
    plt.savefig('./overlay_thin.png', bbox_inches='tight', pad_inches=0)
    plt.close()
...
if __name__ == '__main__':
    ...
    #对距离变换后的图，细化
    mask2_thin = np.where(mask2_dist>0, np.ones_like(mask2_dist, dtype=np.uint8), np.zeros_like(mask2_dist, dtype=np.uint8))
    mask3_thin = np.where(mask3_dist>0, np.ones_like(mask3_dist, dtype=np.uint8), np.zeros_like(mask3_dist, dtype=np.uint8))
    mask4_thin = np.where(mask4_dist>0, np.ones_like(mask4_dist, dtype=np.uint8), np.zeros_like(mask4_dist, dtype=np.uint8))

    dot2s = np.nonzero(mask2_thin)
    dot2s = np.concatenate([dot2s[1].reshape(1,-1).T, dot2s[0].reshape(1,-1).T], axis=1)
    dot3s = np.nonzero(mask3_thin)
    dot3s = np.concatenate([dot3s[1].reshape(1,-1).T, dot3s[0].reshape(1,-1).T], axis=1)
    dot4s = np.nonzero(mask4_thin)
    ...
    #image_, _, _ = line_fitness(dot4s, image, type = cv2.DIST_L1, color=(255, 0, 0))

    # image_2, _, _ = line_fitness(dot5s, image_, type = cv2.DIST_L1, color=(128, 0, 128))

    # h, w, _ = demo_img1.shape
    # cv2.line(demo_img1, (dot21[0]*(w//256),dot21[1]*(h//256)), (dot22[0]*(w//256),dot22[1]*(h//256)), (0, 255, 0), 10)
    # cv2.line(demo_img1, (dot31[0]*(w//256),dot31[1]*(h//256)), (dot32[0]*(w//256),dot32[1]*(h//256)), (255, 255, 0), 10)
    # cv2.line(demo_img1, (dot41[0]*(w//256),dot41[1]*(h//256)), (dot42[0]*(w//256),dot42[1]*(h//256)), (255, 0, 0), 10)
    end_time = time.time()
    print('time cost: ', end_time-start_time)
    image_overlay = np.array(Image.open('./overlay1.png'))
    watch_thin(image, mask2_thin, mask3_thin, mask4_thin)
    image_overlay_thin = np.array(Image.open('./overlay_thin.png'))

    mt.PIS(image, image_overlay, image_overlay_thin, image_)
    #mt.PIS(image, image_overlay, image_overlay_thin, image_, demo_img1)

    ##########################################  总结： ###################################
    #
    #1. L2速度比L1快10倍，但L2的拟合准确度比L1低，特别是直接拟合的导航线。
    #
    #2. 加了细化步骤的拟合准确度在这张图上没有表现出来，弊端明显，拖慢了代码速度。
    #
    ######################################################################################
