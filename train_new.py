...
def get_argparser():
    ...
    #Experiment Note
    parser.add_argument('--note',  type=str, default='all_rows, main_rows(not split), navigation_line Detector',
...
def save_best_img(opts, model, loader, device):
    ...
    with torch.no_grad():
        for j, (images, labels) in tqdm(enumerate(loader)):
            ...
            tgt1, tgt2, tgt3 = lbl3.detach().squeeze(1).cpu().numpy(), lbl5.detach().squeeze(1).cpu().numpy(), lbl4.detach().squeeze(1).cpu().numpy()#(2 256 256), (2 256 256)
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    metrics_1,metrics_2,metrics_5 = metrics
    metrics_1.reset()
    metrics_2.reset()
    metrics_5.reset()
    ret_samples = []
    with torch.no_grad():
        ...
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            metrics_5.update(tgt3, mask3)

            if ret_samples_ids is not None and i in ret_samples_ids:  # get vis samples
                ret_samples.append(
                    (images[0].detach().cpu().numpy(), tgt1[0], mask1[0], tgt2[0], mask2[0], tgt3[0], mask3[0]))

        score1, score2, score3 = metrics_1.get_results(), metrics_2.get_results(), metrics_5.get_results()
...
def main():
    ...
    for cur_epoch in range(opts.epoch_num):
        ...
        for (images, labels) in train_loader:
            ...
            lbls = torch.concat([lbl3, lbl5, lbl4], 1) #all_rows, main_rows, navigation
        ...
        if (cur_epoch) % opts.val_interval == 0:
            ...
            val_score, ret_samples = validate(
                    opts=opts, model=model, loader=val_loader, device=device, metrics=(metrics_1,metrics_2,metrics_5), ret_samples_ids=vis_sample_id)
            print('作物行IOU:\n', metrics_1.to_str(val_score[0]))
            print('主作物行IOU:\n', metrics_2.to_str(val_score[1]))
            print('导航线IOU:\n', metrics_5.to_str(val_score[2]))
            if val_score[2]['Mean IoU'] > best_score:  # when Nevigation_line best
                ...
                if vis is not None:  # visualize validation score and samples
                    ...
                    for k, (img, tgt1, mask1, tgt2, mask2, tgt3, mask3) in enumerate(ret_samples):
                        img = (denorm(img) * 255).astype(np.uint8)
                        tgt1 = train_dst.decode_target(tgt1).transpose(2, 0, 1).astype(np.uint8)
                        mask1 = train_dst.decode_target(mask1).transpose(2, 0, 1).astype(np.uint8)
                        ...
                        concat_img = np.concatenate((img, tgt1, mask1, tgt2, mask2, tgt3, mask3), axis=2)  # concat along width
            ...
            with open(logs_filename, 'a') as f:
                ...
                f.writelines(f'\t导航线   Mean IoU: {miou3} Class IoU: {class_iou3}\n')
        scheduler.step()

    best_info_pp1 = metrics_1.to_str(best_info[0])
    best_info_pp2 = metrics_2.to_str(best_info[1])
    best_info_pp3 = metrics_5.to_str(best_info[2])
    ...
    #Save 'total_params' and 'best_info'
    with open(logs_filename,'a') as f:
        f.write(f'\n\n{Total_Params}\n\n作物行:\n{best_info_pp1}\n\n主作物行:\n{best_info_pp2}\n\n导航线:\n{best_info_pp3}')
    #Print total params
    print(Total_Params)
