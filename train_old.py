...
def get_argparser():
    ...
    #Experiment Note
    parser.add_argument('--note',  type=str, default='two areas detector',
...
def save_best_img(opts, model, loader, device):
    ...
    with torch.no_grad():
        for j, (images, labels) in tqdm(enumerate(loader)):
            ...
            tgt1, tgt2 = lbl3.detach().squeeze(1).cpu().numpy(), lbl2.detach().squeeze(1).cpu().numpy()#(2 256 256), (2 256 256)
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            if ret_samples_ids is not None and i in ret_samples_ids:  # get vis samples
                ret_samples.append(
                    (images[0].detach().cpu().numpy(), tgt1[0], mask1[0], tgt2[0], mask2[0]))

        score1, score2 = metrics_1.get_results(), metrics_2.get_results()
    return (score1, score2), ret_samples
...
def main():
    ...
    for cur_epoch in range(opts.epoch_num):
        ...
        for (images, labels) in train_loader:
            ...
            lbls = torch.concat([lbl3, lbl2], 1) #[2 2 256 256]
        ...
        if (cur_epoch) % opts.val_interval == 0:
            ...
            val_score, ret_samples = validate(
                    opts=opts, model=model, loader=val_loader, device=device, metrics=(metrics_1,metrics_2), ret_samples_ids=vis_sample_id)
            print('作物行IOU:\n', metrics_1.to_str(val_score[0]))
            print('主作物行IOU:\n', metrics_2.to_str(val_score[1]))
            if val_score[1]['Mean IoU'] > best_score:  # save best model
                best_score = val_score[1]['Mean IoU']
                ...
                if vis is not None:  # visualize validation score and samples
                    ...
                    for k, (img, tgt1, mask1, tgt2, mask2) in enumerate(ret_samples):
                        ...
                        concat_img = np.concatenate((img, tgt1, mask1, tgt2, mask2), axis=2)  # concat along width
