...
#ssh -p 23844 root@region-11.autodl.com

def get_argparser():
    parser = argparse.ArgumentParser()

    #Experiment Note
    parser.add_argument('--note',  type=str, default='just detect navigation_line',
    ...
    #Dataset Options
    parser.add_argument('--data_root', type=str, default='/root/autodl-tmp/dataset1',
...
def save_best_img(opts, model, loader, device):
    utils.mkdir('/root/autodl-tmp/record1/checkpoints_%d/results'%(opts.expnum))
    ...
    with torch.no_grad():
        ...
        for j, (images, labels) in tqdm(enumerate(loader)):
            ...
            tgt = lbl4.detach().squeeze(1).cpu().numpy()#(2 256 256)
            ...
            for i in range(len(images)):
                ...
                target = loader.dataset.decode_target(target*3).astype(np.uint8)#将掩码转化为了RGB图像
                pred= loader.dataset.decode_target(pred*3).astype(np.uint8)


                Image.fromarray(image).save('/root/autodl-tmp/record1/checkpoints_%d/results/%d_image.png' % (opts.expnum, img_id))#保存为.png格式的图像
                Image.fromarray(target).save('/root/autodl-tmp/record1/checkpoints_%d/results/%d_target.png' % (opts.expnum, img_id))
                Image.fromarray(pred).save('/root/autodl-tmp/record1/checkpoints_%d/results/%d_pred.png' % (opts.expnum, img_id))
                ...
                plt.savefig('/root/autodl-tmp/record1/checkpoints_%d/results/%d_overlay.png' % (opts.expnum, img_id), bbox_inches='tight', pad_inches=0)
                plt.close()
                img_id += 1
...
def main():
    ...
    # Setup metrics
    metrics = StreamSegMetrics(opts.num_classes + 1)
    ...
    if os.path.exists('/root/autodl-tmp/record1/checkpoints_%d'%opts.expnum):
        print("document repeat!!")
        sys.exit()
    else:
        os.mkdir('/root/autodl-tmp/record1/checkpoints_%d'%opts.expnum)
    # Restore
    best_score = 0.0
    best_info = None
    ...
    for cur_epoch in range(opts.epoch_num):
        ...
        for (images, labels) in train_loader:
            ...
            #lbls = torch.concat([lbl3, lbl2], 1) 
            loss = criterion(outputs, lbl4)
        ...
        if (cur_epoch) % opts.val_interval == 0:
            ...
            if val_score['Mean IoU'] > best_score:  # save best model
                ...
                save_ckpt('/root/autodl-tmp/record1/checkpoints_%d/best_%s.pth' %
                            (opts.expnum, opts.model))
                best_info = val_score
                if vis is not None:  # visualize validation score and samples
                    ...
                    for k, (img, tgt, mask) in enumerate(ret_samples):
                        ...
                        tgt = train_dst.decode_target(tgt*3).transpose(2, 0, 1).astype(np.uint8)
                        mask = train_dst.decode_target(mask*3).transpose(2, 0, 1).astype(np.uint8)
    ...
    best_info_pp = metrics.to_str(best_info)
    print('导航线_best_iou:\n', best_info_pp)

    #Save 'config_info' and 'best_info'
    filename = '/root/autodl-tmp/record1/checkpoints_%d/metrics.txt'%opts.expnum
